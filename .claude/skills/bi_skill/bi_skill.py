...
KB_DIR = WORKSPACE_DIR / "bi"
REPORT_PROFILES_FILE = KB_DIR / "report_profiles_v2.json"
BUSINESS_MAP_FILE = KB_DIR / "kb_bi_business_data_map.json"
FILTER_INVENTORY_FILE = KB_DIR / "filter_learning_inventory.json"
...
def _make_search_parser():
    ...
    p.add_argument("--start-date2", default="", help="第二组开始日期 YYYY-MM-DD")
    p.add_argument("--end-date2", default="", help="第二组结束日期 YYYY-MM-DD")
    p.add_argument("--start-date-field2", default="", help="第二组开始日期筛选器字段名")
    p.add_argument("--end-date-field2", default="", help="第二组结束日期筛选器字段名")
...
def _make_kb_search_parser():
    p = argparse.ArgumentParser(add_help=False)
    p.add_argument("--query", required=True, help="搜索关键词")
    p.add_argument("--limit", type=int, default=10, help="最多返回条数")
    return p


def _make_kb_report_parser():
    p = argparse.ArgumentParser(add_help=False)
    p.add_argument("--name", default="", help="精确报表名称")
    p.add_argument("--query", default="", help="搜索关键词")
    return p


def _make_kb_filters_parser():
    return _make_kb_report_parser()


def _make_kb_modules_parser():
    p = argparse.ArgumentParser(add_help=False)
    p.add_argument("--query", default="", help="模块/关键词")
    p.add_argument("--limit", type=int, default=20, help="最多返回条数")
    return p


# ══════════════════════════════════════════════════════════════════
# 知识库子命令 — 离线查询 BI 报表画像 / 筛选器 / 业务映射
# ══════════════════════════════════════════════════════════════════
...
def _load_report_profiles() -> dict:
    data = _load_json(REPORT_PROFILES_FILE)
    reports = data.get("reports")
    if not isinstance(reports, dict):
        print("[ERROR] report_profiles_v2.json 缺少 reports 对象")
        sys.exit(1)
    return reports


def _report_path_text(profile: dict) -> str:
    path = profile.get("identity", {}).get("path", [])
    if isinstance(path, list):
        return "\\".join(str(p) for p in path if str(p).strip())
    return str(path or "")


def _report_name(profile_name: str, profile: dict) -> str:
    return str(profile.get("identity", {}).get("name") or profile_name)


def _report_search_text(name: str, profile: dict) -> str:
    identity = profile.get("identity", {})
    parts = [
        name,
        _report_name(name, profile),
        _report_path_text(profile),
        str(identity.get("type") or ""),
        str(identity.get("business_domain") or ""),
        str(identity.get("status") or ""),
    ]
    return " ".join(parts).lower()


def _find_report_matches(query: str = "", name: str = "") -> list[tuple[str, dict]]:
    reports = _load_report_profiles()
    if name:
        exact = [(n, p) for n, p in reports.items() if _report_name(n, p) == name or n == name]
        if exact:
            return exact
        query = name
    q = query.strip().lower()
    if not q:
        return []
    matches = [(n, p) for n, p in reports.items() if q in _report_search_text(n, p)]
    matches.sort(key=lambda item: (0 if _report_name(item[0], item[1]).lower() == q else 1, _report_name(item[0], item[1])))
    return matches


def _choose_single_report(query: str = "", name: str = "") -> Optional[Tuple[str, dict]]:
    matches = _find_report_matches(query=query, name=name)
    if len(matches) == 1:
        return matches[0]
    if not matches:
        print(f"[ERROR] 未在离线知识库找到报表: {name or query}")
        return None
    print(f"[ERROR] 匹配到多个报表，请缩小关键词或使用 --name 精确指定（共 {len(matches)} 个）:")
    _print_report_candidates(matches[:10])
    return None


def _print_report_candidates(matches: list[tuple[str, dict]]):
    for i, (name, profile) in enumerate(matches, start=1):
        identity = profile.get("identity", {})
        export = profile.get("export", {})
        print(f"{i}. {_report_name(name, profile)}")
        print(f"   路径: {_report_path_text(profile)}")
        print(f"   类型: {identity.get('type') or '-'} | 导出: {export.get('last_status') or '-'}")


def _schema_columns(profile: dict) -> list[dict]:
    columns = profile.get("schema", {}).get("columns", [])
    return columns if isinstance(columns, list) else []


def _profile_filters(profile: dict) -> list[dict]:
    filters = profile.get("filters", [])
    return filters if isinstance(filters, list) else []


def _date_filter_names(profile: dict, semantic: str) -> list[str]:
    names = []
    for f in _profile_filters(profile):
        if f.get("control_type") == "date" and f.get("semantic") == semantic:
...
def _print_report_summary(name: str, profile: dict):
    identity = profile.get("identity", {})
    export = profile.get("export", {})
    columns = _schema_columns(profile)
    filters = _profile_filters(profile)
    report_name = _report_name(name, profile)
    path = _report_path_text(profile)
    print(f"报表: {report_name}")
    print(f"路径: {path}")
    print(f"类型: {identity.get('type') or '-'} | 状态: {identity.get('status') or '-'} | 权限: {identity.get('authority_level') or '-'}")
    print(f"导出: supported={export.get('supported')} last_status={export.get('last_status') or '-'} bytes={export.get('last_file_bytes') or '-'}")
    print(f"筛选器: {len(filters)} 个 | 字段: {len(columns)} 个 | sheet={profile.get('schema', {}).get('sheet') or '-'}")
    if filters:
        print("\n已知筛选器:")
        for f in filters[:20]:
            changed = "已学习" if f.get("changed") else "默认/未改"
            print(f"- {f.get('label')}: {f.get('control_type')} semantic={f.get('semantic') or '-'} default={f.get('default_value') or '-'} ({changed})")
        if len(filters) > 20:
            print(f"... 另有 {len(filters) - 20} 个筛选器")
    if columns:
        print("\n字段:")
        print(", ".join(str(c.get("name") or "") for c in columns[:40] if c.get("name")))
        if len(columns) > 40:
            print(f"... 另有 {len(columns) - 40} 个字段")
    if path:
        print("\n建议命令:")
        print(f"python3 .claude/skills/bi_skill/bi_skill.py search --path \"{path}\"")


def do_kb_search(args):
    matches = _find_report_matches(query=args.query)
    print(f"匹配报表: {len(matches)} 个，显示前 {min(args.limit, len(matches))} 个")
    _print_report_candidates(matches[:args.limit])


def do_kb_report(args):
    if not args.name and not args.query:
        print("[ERROR] 请指定 --name 或 --query")
        sys.exit(1)
    chosen = _choose_single_report(query=args.query, name=args.name)
    if not chosen:
        sys.exit(2)
    _print_report_summary(*chosen)


def do_kb_filters(args):
    if not args.name and not args.query:
        print("[ERROR] 请指定 --name 或 --query")
        sys.exit(1)
    chosen = _choose_single_report(query=args.query, name=args.name)
    if not chosen:
        sys.exit(2)
    name, profile = chosen
    print(f"报表: {_report_name(name, profile)}")
    print(f"路径: {_report_path_text(profile)}")
    filters = _profile_filters(profile)
    if not filters:
        print("无已知筛选器")
        return
    for f in filters:
        options = f.get("options_sample") or []
        option_text = f" options={options[:8]}" if options else ""
        print(f"- {f.get('label')}: type={f.get('control_type')} semantic={f.get('semantic') or '-'} required={f.get('required')} default={f.get('default_value') or '-'} requested={f.get('requested_value') or '-'} after={f.get('after_value') or '-'} mode={f.get('change_mode') or '-'}{option_text}")


def do_kb_modules(args):
    data = _load_json(BUSINESS_MAP_FILE)
    modules = data.get("modules", [])
    if not isinstance(modules, list):
        print("[ERROR] kb_bi_business_data_map.json 缺少 modules 列表")
        sys.exit(1)
    ...
    for module in modules:
        ...
        if questions:
            print(f"  问题: {'；'.join(str(x) for x in questions[:3])}")
        if keywords:
            print(f"  关键词: {', '.join(str(x) for x in keywords[:12])}")
...
def _preflight_search_with_kb(args):
    if not args.name:
        return
    chosen = _choose_single_report(name=args.name)
    if not chosen:
        sys.exit(2)
    name, profile = chosen
    resolved_path = _report_path_text(profile)
    if not args.path and resolved_path:
        args.path = resolved_path
        print(f"[知识库] 已解析路径: {args.path}")
    if args.start_date and args.start_date_field == "开始时间":
        starts = _date_filter_names(profile, "start_date")
        if len(starts) == 1:
            args.start_date_field = starts[0]
            print(f"[知识库] 已解析开始日期字段: {args.start_date_field}")
        elif len(starts) > 1:
            print(f"[ERROR] 开始日期字段不唯一，请指定 --start-date-field，可选: {starts}")
            sys.exit(2)
    if args.end_date and args.end_date_field == "结束时间":
        ends = _date_filter_names(profile, "end_date")
        if len(ends) == 1:
            args.end_date_field = ends[0]
            print(f"[知识库] 已解析结束日期字段: {args.end_date_field}")
        elif len(ends) > 1:
            print(f"[ERROR] 结束日期字段不唯一，请指定 --end-date-field，可选: {ends}")
            sys.exit(2)


def do_search(args):
    _preflight_search_with_kb(args)
    try:
    ...
    with sync_playwright() as pw:
        ...
        if args.start_date2:
            _apply_date_filter_v2(page, args.start_date_field2, args.start_date2)
        if args.end_date2:
            _apply_date_filter_v2(page, args.end_date_field2, args.end_date2)
...
def _refresh_and_wait(page, max_wait=30):
...
def main():
    if len(sys.argv) < 2:
        print("用法:")
        print("  python3 bi_skill.py kb-search --query <关键词> [--limit 10]")
        print("  python3 bi_skill.py kb-report --name <报表名> | --query <关键词>")
        print("  python3 bi_skill.py kb-filters --name <报表名> | --query <关键词>")
        print("  python3 bi_skill.py kb-modules [--query <关键词>]")
    ...
    if sub == "search":
        ...
        parser = _make_kb_search_parser()
        args, _ = parser.parse_known_args(sys.argv[2:])
        do_kb_search(args)
    elif sub == "kb-report":
        parser = _make_kb_report_parser()
        args, _ = parser.parse_known_args(sys.argv[2:])
        do_kb_report(args)
    elif sub == "kb-filters":
        parser = _make_kb_filters_parser()
        args, _ = parser.parse_known_args(sys.argv[2:])
        do_kb_filters(args)
    elif sub == "kb-modules":
        parser = _make_kb_modules_parser()
        args, _ = parser.parse_known_args(sys.argv[2:])
        do_kb_modules(args)
    else:
        print(f"未知子命令: {sub}")
